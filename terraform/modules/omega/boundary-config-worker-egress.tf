...
data "template_file" "boundary_worker_egress" {
  template = <<-EOT
...
resource "local_file" "boundary_worker_egress" {
  content  = data.template_file.boundary_worker_egress.rendered
  filename = "${path.module}/../secrets/config-boundary-worker-egress.hcl"
}
