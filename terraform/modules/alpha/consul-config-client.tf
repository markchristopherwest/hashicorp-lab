...
data "template_file" "consul_config_client" {
  template = <<-EOT
...
resource "local_file" "consul_config_client" {
  content  = data.template_file.consul_config_client.rendered
  filename = "${path.module}/../secrets/config-consul-client.hcl"
}
