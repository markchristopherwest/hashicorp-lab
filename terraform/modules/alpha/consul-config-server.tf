...
data "template_file" "consul_config_server" {
  template = <<-EOT
...
resource "local_file" "consul_config_server" {
  content  = data.template_file.consul_config_server.rendered
  filename = "${path.module}/../secrets/config-consul-server.hcl"
}
