...
data "template_file" "tfe_config_server" {
  template = <<-EOT
...
resource "local_file" "tfe_config_server" {
  content  = data.template_file.tfe_config_server.rendered
  filename = "${path.module}/../secrets/config-tfe-values.yaml"
}
